...
const RegisterAlone = () => {
    ...
    const registerUser = async () => {
        ...
        try {
            await api.post(`/api/register-alone/`, craftRequestBody());
            /** auto login */
            try {
                ...
                navigate('/register-alone/welcome', {
                    state: {
                        ...location.state,
                        verifiedEmail: null,
                        verifiedPassword: null,
                        isRegisterAloneProcess: null,
                        isRegisterAloneDone: true,
                        isAutoLogin: true,
                        newComerName: name
                    }
                });
    ...
    const handleBack = () => {
        ...
        /** Warning : The registration will be reset */
    ...
    useEffect(() => {
        if (!isValidFlow) {
            let isProcessValid = location?.state?.isRegisterAloneProcess ?
                location?.state?.isRegisterAloneProcess : false;
            ...
            const isDataValid = (givenEmail.trim() !== "") && (givenPassword.trim() !== "");
        ...
        <div className='exp-body exp-with-pd-28'>
            <div className="exp-container">
                ...
                <ConfirmPopUp
                    ...
                    setIsOpen={setIsWarningConfirmOpen}
                    message={"회원가입을 취소하시겠어요?"}
                    onConfirm={() => {
                        setIsWarningConfirmOpen(false);
                        navigate('/login');
                    }}
                    onReject={() => setIsWarningConfirmOpen(false)}
                    confirmLabel={"네"}
                    rejectLabel={"아니요"}
                    isCrucial={true}
                />
