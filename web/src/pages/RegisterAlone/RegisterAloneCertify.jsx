...
const RegisterAloneCertify = () => {
    ...
    const isDomainValid = (text) => {
        return (
            text.endsWith("@kaist.ac.kr") ||
            text.endsWith("@alumni.kaist.ac.kr")
        );
    };
    ...
    return (
        <div className="exp-body exp-with-pd-28">
            ...
            {/** Title */}
            <div className="exp-title-container">{"회원 가입"}</div>
            {/** Email */}
            <div className="exp-inv-cer-field-container mt-32">
                <span className="exp-inv-cer-field-title">{"학교 이메일"}</span>
                ...
                <div className="exp-inv-cer-email-input-row">
                    <input
                        className="exp-inv-cer-email-input exp-inv-input-dynamic"
                        readOnly={
                            !isCodeSent || isCodeVerified || isSendLoading
                        }
                        value={inputCode}
                    ...
                    <button
                        ...
                        {isCodeVerified ? "확인됨" : "인증코드 확인"}
                    </button>
                </div>
                {isCodeVerified != null && (
                    <InfoMessage
                        type={isCodeVerified ? "good" : "bad"}
                        message={
                            isCodeVerified
                                ? "인증코드가 일치합니다."
                                : "인증코드가 일치하지 않습니다."
            ...
            <div className="exp-inv-cer-field-container mt-15">
                <span className="exp-inv-cer-field-title">{"비밀번호"}</span>
                <div className="exp-inv-cer-email-input-row">
                    <input
            ...
            <div className="exp-inv-cer-field-container mt-20">
                <button
                    className={`exp-save-button no-mg${
                        !isCodeVerified || !isPasswordSame
                            ? " exp-btn-disabled"
                            : ""
                    }`}
                    onClick={() => {
                        if (isCodeVerified && isPasswordSame) {
                            navigate(`/register-alone/register`, {
                                state: {
                                    ...location.state,
                                    isRegisterAloneProcess: true,
                                    verifiedEmail: email,
...
export default RegisterAloneCertify;
