...
const CommentSection = ({ project, comments, setComments, currentUser }) => {
    ...
    const updateComment = (commentId) => {
        ...
        // PATCH 요청으로 수정된 댓글/답글 데이터를 서버에서 받아옴
        api.patch(`/api/comments/${commentId}/edit/`, { content: editContent })
            .then((res) => {
                // 댓글 목록을 업데이트하되, 수정된 댓글/답글만 업데이트
                setComments(comments.map((comment) => {
                    // 수정된 댓글인 경우 데이터를 업데이트
                    if (comment.id === commentId) {
                        return res.data;
                    }
                    // 답글이 있는 경우 답글 중에서 수정된 답글 찾아 업데이트
                    if (comment.replies) {
                        return {
                            ...comment,
                            replies: comment.replies.map((reply) => 
                                reply.id === commentId ? res.data : reply
                            ),
                        };
                    }
                    return comment;
    ...
    const deleteComment = (commentId) => {
        api.delete(`/api/comments/${commentId}/delete/`)
            .then(() => {
                setComments((prevComments) => {
                    // 댓글과 답글에서 삭제 대상 댓글을 재귀적으로 삭제
                    const removeCommentRecursively = (commentsList, commentIdToDelete) => {
                        return commentsList.filter((comment) => {
                            // 해당 댓글이 삭제 대상인 경우 false 반환 (즉, 삭제)
                            if (comment.id === commentIdToDelete) {
                                return false;
                            }
    
                            // 답글이 있는 경우 재귀적으로 답글 내에서 삭제
                            if (comment.replies && comment.replies.length > 0) {
                                comment.replies = removeCommentRecursively(comment.replies, commentIdToDelete);
                            }
    
                            return true; // 그 외 댓글은 유지
                        });
                    };
    
                    // 최상위 댓글 리스트에 대해 재귀적으로 삭제 처리
    ...
    const renderComments = (commentList, iscomment) => {
        return commentList.map((comment) => (
            <div key={comment.id} className="project-comment">
                <img 
                    src={profileImages[comment.user] ? profileImages[comment.user] : defaultProfileImage} 
                    alt={comment.user_name} 
                    className="project-comment-profile-img" 
                />
                <div className="project-comment-content">
                    ...
                    {comment.replies && comment.replies.length > 0 && (
                        <div className="project-replies">
                            {renderComments(comment.replies, 0)} {/* Recursive call */}
                        </div>
                    )}
                </div>
            </div>
    ...
    return (
        <div className="project-comment-section">
            <div className="project-add-comment" onClick={handleAddCommentClick}>
                <img 
                    src={currentUser.profile.image ? currentUser.profile.image : defaultProfileImage} 
                    alt={`${currentUser.profile.user_name}'s profile`} 
                    className="project-my-profile-img" 
                />
                <input 
                    type="text" 
                    placeholder="댓글 작성하기..." 
            ...
            {renderComments(filteredComments, 1)}
            {/* 댓글 작성하기 */}
            {showCommentInput && (
