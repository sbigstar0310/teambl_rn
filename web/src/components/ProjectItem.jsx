...
function ProjectItem({ project, onDelete, currentUser, refreshProjects }) {
  ...
  const [editContact, setEditContact] = useState(project.contacts || []);

  const [allFriends, setAllFriends] = useState([]);
  const [editTaggedUsers, setEditTaggedUsers] = useState(
    Array.isArray(project.tagged_users) ? project.tagged_users : []
  );
  ...
  useEffect(() => {
    // 기존 이미지를 미리보기 설정
    const existingImagePreviews = project.images.map(
  ...
  useEffect(() => {
    // 프로젝트의 댓글 목록을 불러오는 함수
    api
      .get(`/api/projects/${project.project_id}/comments/`)
      .then((res) => {
    ...
    const fetchLikedStatus = async () => {
      try {
        const response = await api.get(
          `/api/projects/${project.project_id}/liked-status/`
        );
  ...
  const openModal = () => {
    ...
    setFilteredFriends(
      allFriends.filter((user) =>
        editTaggedUsers.some((taggedUser) => taggedUser.id === user.id)
      )
    );
  ...
  const confirmTaggedUsers = () => {
    const updatedTaggedUsers = allFriends.filter((user) =>
      selectedUserIds.includes(user.id)
    );
  ...
  const addTaggedUser = () => {
    if (
      selectedUser &&
      !editTaggedUsers.some((user) => user.id === selectedUser)
    ) {
      const userObj = allFriends.find(
  ...
  const removeTaggedUser = (userId) => {
    const updatedTaggedUsers = editTaggedUsers.filter(
      (user) => user.id !== userId
    );
    setEditTaggedUsers(updatedTaggedUsers);
  };
  ...
  const getTotalUniqueCommentsCount = (comments) => {
    const uniqueIds = new Set();

    // 댓글과 각 댓글의 답글에서 고유한 ID를 찾아 Set에 추가
    comments.forEach((comment) => {
      uniqueIds.add(comment.id); // 댓글의 ID 추가

      if (comment.replies && comment.replies.length > 0) {
        // 답글이 있을 경우, 각 답글의 ID 추가
  ...
  const saveProject = () => {
    ...
    editContact.forEach((contact) => {
      formData.append("contacts[]", contact.contact_info);
    });
    ...
    api
      .patch(`/api/projects/${project.project_id}/edit/`, formData, {
        headers: {
      ...
      .then((res) => {
        ...
        setEditContact(res.data.contacts);
  ...
  const cancelEdit = () => {
    ...
    setEditContact(project.contacts || []);
  };
    ...
    <div className="project-container">
      <div className="new-project" key={project.project_id}>
        ...
        <Reactions
          ...
          likes={likeCount}
          commentsLength={getTotalUniqueCommentsCount(comments)}
        ...
        <div>
          ...
          <>
            ...
            <div className="contact-list">
              <strong>기존 연락처</strong>
              {editContact.length > 0 ? (
                <ul>
                  {editContact.map((contact, index) => (
                    <li key={index}>
                      ...
                      <button
                        ...
                        onClick={
                          () =>
                            setEditContact(
                              editContact.filter((_, i) => i !== index)
                            ) // 연락처 삭제
                        }
                      >
                        Remove
