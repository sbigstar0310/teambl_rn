  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (method === "login") {
        ...
        /** NEW : when from invitation */
        if ((location.state != null) && location.state.isExpProcess) {
          if ((location.state.expTargetUser != null) && location.state.expTargetUser != res.data.userId) {
            navigate(`/home`);
          } else {
            navigate(`/experience/welcome/${location.state.expInvitationCode}`);
          }
        } else {
          navigate(`/home`);
        }
  ...
  return (
    ...
    <p className="login-subheader">팀원 찾기의 새로운 기준, 팀블!</p>
      <div className="login-form">
        ...
        {/* NEW : sign up */}
        <div className="alone-register">
          <span onClick={() => navigate('/register-alone/certify')}>{"회원가입"}</span>
        </div>
      </div>
    </div>
  );
