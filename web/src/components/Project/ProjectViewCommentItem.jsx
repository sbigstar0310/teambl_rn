...
import ProjectViewReplyItem from './ProjectViewReplyItem';
import { toastText } from '../Toast/Toast';

const MAX_HEIGHT = 30;

const ProjectViewCommentItem = ({ projectId, commentInfo, updateCommentList }) => {
    ...
    const deleteComment = async () => {
        ...
        try {
            ...
            toastText("댓글이 삭제되었습니다.");
        } catch (e) {
            toastText("댓글 삭제에 실패했습니다.");
            console.log(e);
        } finally {
    ...
    const putComment = async () => {
        ...
        try {
            ...
            toastText("댓글이 수정되었습니다.");
        } catch (e) {
            toastText("댓글 수정에 실패했습니다.");
            console.log(e);
        } finally {
    ...
    const postReply = async () => {
        ...
        try {
            ...
            await updateCommentList();
            await setIsReplySectionOpen(true);
            if (commentRef.current) {
                commentRef.current.scrollIntoView({ behavior: 'smooth' });
            }
        } catch (e) {
            console.log(e);
            toastText("답글은 300자 제한입니다.");
        } finally {
    ...
    useEffect(() => {
        getRelationshipDegree(commentInfo['user']);
    }, []);
    ...
    return (
        ...
        <div
            className={
                `projectView-commentItem-container`
                + `${isReplySectionOpen ? ' projectView-commentItem-bp-0' : ''}`
            }
            ref={commentRef}
        >
            {/** profile image */}
            ...
            <div className='projectView-commentItem-content-container'>
                ...
                <div className='projectView-commentItem-1st-row-container'>
                    ...
                    {
                        (isDataLoaded) &&
                        <div className='projectView-commentItem-name-relation-time-container'>
                            <span className='projectView-commentItem-name'>
                                {userInfo.profile.user_name}
                            </span>
                            {
                                (!(commentInfo['user'] === currentUserInfo?.id))&& (!(isLoading)) &&
                                <span className='projectView-commentItem-relation'>{` ・ ` + (relationshipDegree ? `${relationshipDegree}촌` : "4촌 이상")}</span>
                            }
                    ...
                    {/** menu button */}
                    {
                        (commentInfo['user'] == currentUserInfo?.id) &&
                        <button
        ...
        {/** reply view */}
        {
            ...
            (!isReplySectionOpen) &&
            <div className='projectView-commentItem-reply-open-button-container'>
                <button
                    className='projectView-commentItem-reply-open-button'
                    onClick={() => setIsReplySectionOpen(true)}
                >
                    {`답글 ${commentInfo['replies'].length}개 보기`}
                </button>
            </div>
        }
        {
            ...
            (commentInfo['replies'].length > 0) &&
            isReplySectionOpen &&
            commentInfo['replies'].map((replyInfo) => {
                return (
                    <ProjectViewReplyItem
                        key={replyInfo['id']}
                        projectId={projectId}
        ...
        }
        {
            ...
            <div className='projectView-commentItem-reply-open-button-container'>
                <button
                    className='projectView-commentItem-reply-open-button'
                    onClick={() => setIsReplySectionOpen(false)}
                >
                    {`답글 닫기`}
                </button>
            </div>
...
export default ProjectViewCommentItem;