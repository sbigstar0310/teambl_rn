...
const InquirySend: React.FC = () => {
  const [currentUserInfo, setCurrentUserInfo] = useState<{ email?: string }>({ email: "user@example.com" });
  ...
  const postNewInquiry = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        setContent("");
        Alert.alert("성공", "문의가 정상적으로 접수되었습니다. 확인하여 빠른 시일 내에 답변 드리겠습니다.");
        setIsLoading(false);
      }, 1000);
    } catch (e) {
      Alert.alert("실패", "문의 등록에 실패했습니다. 서버 오류가 발생했습니다.");
      console.log(e);
      setIsLoading(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
      {/** from to view */}
      <Text style={styles.emailText}>{`받는 사람: ${TEAMBLE_EMAIL}`}</Text>
      ...
      {/** button */}
      <TouchableOpacity
        style={[styles.button, !(isLoading || isDataLoading || content === "") ? styles.buttonActive : styles.buttonDisabled]}
